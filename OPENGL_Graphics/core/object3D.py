...
class Object3D(object):
    ...
    def __init__(self):
        self.transform=Matrix.makeIdenity()
        self.parent=None
        self.children=[]
    ...
    def getDescendantList(self):
        # master list of all descentdants
        descendants=[]
        # node to be added to descendant list,
        # and whose children will be added to this list
        nodeToProcess=[self]
        # continue processing nodes while any are left
        while len(nodeToProcess)>0:
            # remove first node from list 
            node=nodeToProcess.pop(0)
            # add node to scendants.append(nodeToProcess)
            descendants.append(node)
            # children of this node must also be processed
            nodeToProcess=node.children+nodeToProcess
        return descendants
